    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['or_text'] = $fetched_data['text'];
             ...
             // ФІКС: Дешифруємо ПЕРЕД обробкою Wo_Markup/Wo_Emo!
            ...
            $is_browser = empty($_POST['app_version']) && empty($_GET['app_version']) && !$is_worldmates;

 

            // Если браузер - ДЕШИФРУЄМО текст в plain text

            if ($is_browser) {

                $crypto_path = dirname(__DIR__, 2) . '/api/v2/crypto_helper.php';
                ...
                if (class_exists('CryptoHelper')) {
                    ...
                    try {
                        ...
                        $has_gcm = !empty($fetched_data['iv']) && !empty($fetched_data['tag']);

                        $has_ecb = !empty($fetched_data['text_ecb']);

 

                        if ($has_gcm) {

                            // Дешифруємо GCM → plain text
                            ...
                            if ($decrypted !== false) {
                                ...
                                $fetched_data['text'] = $decrypted;

                                $fetched_data['or_text'] = $decrypted;

                            }

                        } elseif ($has_ecb) {

                            // Дешифруємо ECB → plain text

                            $decrypted = CryptoHelper::decryptECB($fetched_data['text_ecb'], $fetched_data['time']);
                        ...
                        error_log("Wo_GetMessages decrypt error: " . $e->getMessage());
                ...
                unset($fetched_data['iv'], $fetched_data['tag'], $fetched_data['cipher_version'], $fetched_data['text_ecb']);

            }

 

            // ТЕПЕР обробляємо вже дешифрований текст
...
include_once($_SERVER['DOCUMENT_ROOT'] . '/assets/libraries/SimpleImage-master/vendor/claviska/simpleimage/src/claviska/SimpleImage-Class.php');
