...
class CryptoHelper {
     ...
     */
    public static function decryptGCM($ciphertext, $iv, $tag, $timestamp) {
        try {
            // Декодируем Base64
            ...
            if ($plaintext === false) {
                error_log("CryptoHelper: GCM decryption failed (authentication failed)");
                return false;
            }
    ...
    public static function decrypt($ciphertext, $timestamp, $iv = null, $tag = null, $cipherVersion = null) {
        ...
        if ($cipherVersion === self::CIPHER_VERSION_GCM || ($iv !== null && $tag !== null)) {
            // GCM режим
            if ($iv !== null && $tag !== null) {
                return self::decryptGCM($ciphertext, $iv, $tag, $timestamp);
            } else {
