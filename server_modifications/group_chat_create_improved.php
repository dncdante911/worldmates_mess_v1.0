<?php
/**
 * ПОКРАЩЕНА СЕКЦІЯ ДЛЯ СТВОРЕННЯ ГРУПОВИХ ЧАТІВ
 *
 * Цей код замінює секцію if ($_POST['type'] == 'create') в оригінальному group_chat.php
 *
 * ПОКРАЩЕННЯ:
 * - Детальне логування всіх параметрів та помилок
 * - Краща валідація даних
 * - Зрозумілі повідомлення про помилки
 * - Обробка порожнього parts
 * - Логування результату Wo_CreateGChat()
 *
 * ІНСТРУКЦІЯ:
 * 1. Зробіть бекап оригінального /var/www/www-root/data/www/worldmates.club/api/v2/endpoints/group_chat.php
 * 2. Замініть секцію if ($_POST['type'] == 'create') { ... } на цей код
 * 3. Перевірте що шлях до лог-файлу існує та має права на запис
 */

// Шлях до лог-файлу (створіть директорію якщо не існує)
...
if ($_POST['type'] == 'create') {
    ...
    log_debug("User ID: " . $wo['user']['id'], $log_file);
    ...
    if (empty($error_code)) {
        ...
        if (empty($parts_raw) || trim($parts_raw) == '') {
            $users = array($wo['user']['id']);
            log_debug("Parts was empty, using only current user: {$wo['user']['id']}", $log_file);
        } else {
