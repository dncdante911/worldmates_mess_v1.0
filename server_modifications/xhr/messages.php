...
function convertMessagesForBrowser(&$messages) {
    ...
    foreach ($messages as &$message) {
        ...
        // ФІКС: Браузер потребує ДЕШИФРОВАНИЙ текст (plain text), а не зашифрований ECB!
        ...
        if ($has_gcm && class_exists('CryptoHelper')) {
            // Сообщение зашифровано GCM - дешифруємо
            try {
                ...
                if ($decrypted !== false) {
                    $message['text'] = $decrypted; // PLAIN TEXT для браузера
                } else {
                    error_log("convertMessagesForBrowser: Failed to decrypt GCM message ID " . ($message['id'] ?? 'unknown'));
                }
            } catch (Exception $e) {
                error_log("convertMessagesForBrowser GCM error: " . $e->getMessage());
            }
        } elseif ($has_ecb && class_exists('CryptoHelper')) {
            // Сообщение зашифровано ECB - дешифруємо
            try {
                $decrypted = CryptoHelper::decryptECB($message['text_ecb'], $timestamp);

                if ($decrypted !== false) {
                    $message['text'] = $decrypted; // PLAIN TEXT для браузера
                } else {
                    error_log("convertMessagesForBrowser: Failed to decrypt ECB message ID " . ($message['id'] ?? 'unknown'));
                }
            } catch (Exception $e) {
                error_log("convertMessagesForBrowser ECB error: " . $e->getMessage());
            }
        }

        // Удаляем технические поля шифрования для браузера
